 ...
 * firing cells. A firing cell becomes 'dying' cell, and a dying cell becomes an off cell 
 * 
...
public class BriansBrain implements CellularAutomaton {
	...
	/*
	 * 0 -- Off/Dead cells are BLACK 
	 * 1 -- Firing cells are RED 
	 * 2 -- Dying/Refractory cells are BLUE
	 */
	...
	public int evalCell(int[][] neighborhood) {
		...
		if(mid==1) return 2; //firing cells go to "dying"
		if(mid==2) return 0; //dying cells go to "off"
		if(mid==0){
			for(i=0; i<3; i++){
				for(j=0; j<3; j++){
					if(neighborhood[i][j]==1) {
						numfiring++;
						if(numfiring>2) return 0;
					}
				}
			}
			if(numfiring == 2) return 1;
		}
		return 0;
	}
