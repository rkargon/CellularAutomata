...
public class Generations implements CellularAutomaton {
	...
	private int born, survive, states;
	private Color[] cols;
	...
	public int evalCell(int[][] neighborhood) {
		...
		for(i=0; i<3; i++){
			for(j=0; j<3; j++){
				if(neighborhood[i][j]==1) sum++;
			}
		}
		if(mid==1){
			if((survive & (1<<(sum-1))) != 0) return 1;//sum-1 is used to account for middle square
			else return 2%states;
		}
		if(mid==0){
			if((born & (1 << sum)) != 0) return 1;
			else return 0;
		}
		return 0;
	}
