...
public class CAPanel extends ZoomablePanel {
	 ...
	 * 
	 * <code>prevmode</code> stores the previous mode, so user can easily switch
	 * back and forth.
	 */
	private int mode = 0;
	private int prevmode = 1; // user can switch to edit by default, will not be
								// necessary when GUI is built
	...
	public void paintCA() {
		...
		for (int i = 0; i < ca.getWidth(); i++) {
			for (int j = 0; j < ca.getHeight(); j++) {
				if (grid[i][j] != 0)
					this.addShape(
							new Rectangle(i + ca.getXOffset(), j
									+ ca.getYOffset(), 1, 1),
							ca.getColorPoint(i + ca.getXOffset(),
									j + ca.getYOffset()), true);
			}
		}
		repaint();
	}
	...
	public void paintComponent(Graphics g) {
		...
		if (this.getDelta() <= 0.062) {
			g2d.setColor(Color.LIGHT_GRAY);
			double delta = this.getDelta(); // x,y coordinates per pixel
			int min_x = (int) getXYCoordinates(new Point2D.Double(0, 0)).x - 1;
			int min_y = (int) getXYCoordinates(new Point2D.Double(0, 0)).y + 1;
			int left = (int) getPixelCoordinates(new Point2D.Double(min_x,
					min_y)).x;
	...
	private class CAMouseAdapter extends ViewMouseAdapter {
		public void mouseClicked(MouseEvent e) {
			switch (mode) {
			case 1:
				Point2D.Double p = getXYCoordinates(new Point2D.Double(
						e.getX(), e.getY()));
	...
	private class CAMouseMotionAdapter extends MouseMotionAdapter {
		...
		public void mouseDragged(MouseEvent e) {
			if (e.getX() > getWidth())
				translatePixels(new Point(10, 0));
			else if (e.getX() < 0)
				translatePixels(new Point(-10, 0));
			if (e.getY() > getHeight())
				translatePixels(new Point(0, 10));
			else if (e.getY() < 0)
				translatePixels(new Point(0, -10));
